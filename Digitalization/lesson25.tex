...
% Section 174
\newSection The translative case (called \m{saav} or `becoming' in Estonian) as such indicates what someone or something is turning into. It answers the questions \m{kelleks?} `becoming whom?' and \m{milleks?} `becoming what?'. In other words, the translative case expresses a change in identity or condition. \\
...
% Section 177
\newSection c) to indicate the time during which or by which something occurs. Here, the translative case answers the questions \m{kui kauaks?} `for how long' and \m{mis ajaks?} `by when?'.
...
% Section 182
\newSection Personal pronouns are declined regularly in the translative case. This is, the \m{-ks} ending is added to the genitive form:
...
% Section 183
\newSection The emphatic particle \m{-ki} or \m{-gi} can be added to almost any word to which you want to give special emphasis or draw special attention. Which of these two forms is used depends on the letter at the end of the word to which the particle is added. \\
...
% Section 184
\newSection The particle \m{-ki} or \m{-gi} can often be translated into English as `even' or `indeed'. In a negative sentence, it can mean `(not) even' or `(not/none) at all' Sometimes, however, it is impossible to give an exact counterpart in English. \\
...
% Section 185
\newSection In some cases, the emphatic particle has become a permanent part of the word: \m{siiski} `even so', \m{iialgi} `ever', \m{kunagi} `sometime', \m{ei kunagi} `never', \m{isegi} `even', \m{keegi} `someone', \m{miski} `something', \m{ei ükski} `no one', \m{kumbki} `either (one)'. \\
